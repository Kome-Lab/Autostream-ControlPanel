...
func TestHostSelfUpdateHTTPPolicyAndGrantRemainExactlyBound(t *testing.T) {
	...
	fence := updateagent.LocalExecutorMutationFence{
		SourcePolicyRevision:    policy.SourcePolicyRevision,
		OwnershipEpoch:          policy.OwnershipEpoch,
		OwnershipPolicyRevision: policy.Revision,
		ExecutorPolicyRevision:  policy.LocalExecutorPolicyRevision,
	}
	stagePlanSHA256, err := updateagent.HostSelfUpdateGrantPlanSHA256(
		"stage",
		updateagent.HostAgentPolicy{
			SelfUpdateID:       policy.SelfUpdateID,
			SelfUpdateRevision: policy.SelfUpdateRevision,
		},
		policy.SelfUpdate.HostSelfUpdateRequest,
		fence,
	)
	if err != nil {
		t.Fatal(err)
	}
	panelServer := httptest.NewServer(handler)
	defer panelServer.Close()
	panelClient := updateagent.PanelClient{
		BaseURL: panelServer.URL,
		Token:   token.RawToken,
		HTTP:    panelServer.Client(),
	}
	stageAuthorization, err := panelClient.IssueHostSelfUpdateGrant(
		t.Context(),
		updateagent.HostSelfUpdateGrantIssueRequest{
			SelfUpdateID:     policy.SelfUpdateID,
			ExpectedRevision: policy.SelfUpdateRevision,
			Operation:        "stage",
			PlanSHA256:       stagePlanSHA256,
			SessionID:        "session-e2e-stage",
		},
	)
	if err != nil {
		t.Fatalf(
			"actual Host Agent stage grant issue: %v release=%#v",
			err,
			policy.SelfUpdate.Release,
		)
	}
	if stageAuthorization.Binding.PlanSHA256 != stagePlanSHA256 ||
		stageAuthorization.Binding.Release.ManifestAssetID !=
			policy.SelfUpdate.Release.ManifestAssetID ||
		stageAuthorization.Binding.Release.MinimumPanelVersion !=
			policy.SelfUpdate.Release.MinimumPanelVersion {
		t.Fatalf(
			"actual stage grant lost plan/release binding: %#v",
			stageAuthorization.Binding,
		)
	}
	stageReceipt, err := updateagent.ConsumeHostSelfUpdateGrant(
		t.Context(),
		panelServer.URL,
		stageAuthorization,
	)
	if err != nil || !stageReceipt.Consumed {
		t.Fatalf(
			"actual root stage grant consume: receipt=%#v err=%v",
			stageReceipt,
			err,
		)
	}
	...
	if err != nil ||
		...
		reserved.Revision != policy.SelfUpdateRevision+1 ||
		stageReceipt.Grant.StageClaimRevision != reserved.Revision ||
		stageReceipt.Grant.StageClaimedAt == nil ||
		stageReceipt.Grant.ConsumedAt == nil ||
		!stageReceipt.Grant.StageClaimedAt.Equal(
			*stageReceipt.Grant.ConsumedAt,
		) {
		t.Fatalf(
			"HTTP/root consume did not durably claim staging: update=%#v receipt=%#v err=%v",
			reserved,
			stageReceipt,
			err,
		)
	...
	for name, revision := range map[string]int64{
		"stale_pre_claim": policy.SelfUpdateRevision,
		"claimed":         reserved.Revision,
	} {
		cancelRequest := httptest.NewRequest(
			http.MethodPost,
			"/system-updates/host-self-updates/"+created.ID+"/cancel",
			strings.NewReader(
				`{"expected_revision":`+jsonInteger(revision)+`}`,
			),
		)
		...
		if cancelResponse.Code != http.StatusConflict {
			t.Fatalf(
				"%s cancel crossed stage claim: status=%d body=%s",
				name,
				cancelResponse.Code,
	...
	if _, err := panelClient.IssueHostSelfUpdateGrant(
		t.Context(),
		updateagent.HostSelfUpdateGrantIssueRequest{
			SelfUpdateID:     policy.SelfUpdateID,
			ExpectedRevision: policy.SelfUpdateRevision,
			Operation:        "stage",
			PlanSHA256:       "sha256:" + stagePlanSHA256,
			SessionID:        "session-e2e-prefixed",
		},
	); err == nil {
		t.Fatal("Host Agent accepted a prefixed grant plan digest")
	}
	prefixedHTTPResponse := httptest.NewRecorder()
	prefixedHTTPRequest := httptest.NewRequest(
		http.MethodPost,
		"/services/host-agent/self-updates/"+created.ID+"/grants",
		strings.NewReader(
			`{"expected_revision":`+
				jsonInteger(policy.SelfUpdateRevision)+
				`,"operation":"stage","plan_sha256":"sha256:`+
				stagePlanSHA256+
				`","session_id":"session-http-prefixed"}`,
		),
	)
	prefixedHTTPRequest.Header.Set(
		"Authorization",
		"Bearer "+token.RawToken,
	)
	handler.ServeHTTP(prefixedHTTPResponse, prefixedHTTPRequest)
	if prefixedHTTPResponse.Code != http.StatusBadRequest {
		t.Fatalf(
			"HTTP accepted prefixed grant digest: status=%d body=%s",
			prefixedHTTPResponse.Code,
			prefixedHTTPResponse.Body.String(),
		)
	}
	mixedAuthorization := stageAuthorization
	mixedAuthorization.Binding.PlanSHA256 = "sha256:" + stagePlanSHA256
	if _, err := updateagent.ConsumeHostSelfUpdateGrant(
		t.Context(),
		panelServer.URL,
		mixedAuthorization,
	); err == nil {
		t.Fatal("root consume accepted a prefixed/bare mixed grant")
	}

	rawStageToken := stageAuthorization.Token.Reveal()
	consumeBody, err := json.Marshal(map[string]any{
		"token":   rawStageToken,
		"binding": stageAuthorization.Binding,
	})
	if err != nil {
		t.Fatal(err)
	}
	wrongHostBinding := stageAuthorization.Binding
	wrongHostBinding.ExecutionHostID = "host-b"
	wrongHostBody, err := json.Marshal(map[string]any{
		"token":   rawStageToken,
		"binding": wrongHostBinding,
	})
	if err != nil {
		t.Fatal(err)
	}
	wrongHostRequest := httptest.NewRequest(
		http.MethodPost,
		"/services/host-agent/self-update-grants/consume",
		strings.NewReader(string(wrongHostBody)),
	)
	wrongHostResponse := httptest.NewRecorder()
	handler.ServeHTTP(wrongHostResponse, wrongHostRequest)
	if wrongHostResponse.Code != http.StatusBadRequest {
		t.Fatalf(
			"wrong host binding status=%d body=%s",
			wrongHostResponse.Code, wrongHostResponse.Body.String(),
		)
	}
	consumeRequest := httptest.NewRequest(
		http.MethodPost,
		"/services/host-agent/self-update-grants/consume",
		strings.NewReader(string(consumeBody)),
	)
	consumeResponse := httptest.NewRecorder()
	handler.ServeHTTP(consumeResponse, consumeRequest)
	if consumeResponse.Code != http.StatusOK ||
		consumeResponse.Header().Get("Cache-Control") != "no-store" {
		t.Fatalf(
			"consume status=%d body=%s",
			consumeResponse.Code, consumeResponse.Body.String(),
		)
	}
	if strings.Contains(consumeResponse.Body.String(), rawStageToken) {
		t.Fatalf("consume echoed raw grant: %s", consumeResponse.Body.String())
	}
	var consumed store.ConsumeSystemUpdateHostSelfUpdateGrantResult
	if err := json.NewDecoder(consumeResponse.Body).Decode(&consumed); err != nil {
		t.Fatal(err)
	}
	// The grant was already consumed through the actual root client above;
	// this direct HTTP call is the exact response-loss replay.
	if consumed.Consumed ||
		consumed.Grant.ExecutionHostID != created.ExecutionHostID ||
		consumed.Grant.StageClaimRevision != reserved.Revision {
		t.Fatalf("consume lost exact host binding: %#v", consumed)
	}
	...
	if replayResponse.Code != http.StatusOK {
		t.Fatalf(
			"consume replay status=%d body=%s",
			replayResponse.Code, replayResponse.Body.String(),
		)
	}
	var replayed store.ConsumeSystemUpdateHostSelfUpdateGrantResult
	if err := json.NewDecoder(replayResponse.Body).Decode(&replayed); err != nil {
		t.Fatal(err)
	}
	if replayed.Consumed || replayed.Grant.ConsumedAt == nil ||
		replayed.Grant.StageClaimRevision != reserved.Revision ||
		replayed.Grant.StageClaimedAt == nil {
		t.Fatalf("consume response-loss replay was not reconciled: %#v", replayed)
	}

	staged, _, err := updates.ObserveSystemUpdateHostSelfUpdate(
		t.Context(),
	...
	if err != nil {
		t.Fatalf("stage observation for reconcile grant: %v", err)
	}
	reconcilePlanSHA256, err := updateagent.HostSelfUpdateGrantPlanSHA256(
		"reconcile",
		updateagent.HostAgentPolicy{
			SelfUpdateID:       staged.ID,
			SelfUpdateRevision: staged.Revision,
		},
		policy.SelfUpdate.HostSelfUpdateRequest,
		fence,
	)
	...
	reconcileAuthorization, err := panelClient.IssueHostSelfUpdateGrant(
		t.Context(),
		updateagent.HostSelfUpdateGrantIssueRequest{
			SelfUpdateID:     staged.ID,
			ExpectedRevision: staged.Revision,
			Operation:        "reconcile",
			PlanSHA256:       reconcilePlanSHA256,
			SessionID:        "session-e2e-reconcile",
		},
	)
	if err != nil {
		t.Fatalf("actual Host Agent reconcile grant issue: %v", err)
	}
	reconcileReceipt, err := updateagent.ConsumeHostSelfUpdateGrant(
		t.Context(),
		panelServer.URL,
		reconcileAuthorization,
	)
	if err != nil || !reconcileReceipt.Consumed ||
		reconcileReceipt.Grant.PlanSHA256 != reconcilePlanSHA256 ||
		!sameHostSelfUpdateReleaseBindingForHTTPTest(
			reconcileReceipt.Grant.Release,
			staged.Release,
		) {
		t.Fatalf(
			"actual root reconcile grant consume lost binding: receipt=%#v err=%v",
			reconcileReceipt,
			err,
		)
	}
...
func sameHostSelfUpdateReleaseBindingForHTTPTest(
	got updateagent.HostSelfUpdateReleaseIdentity,
	want store.SystemUpdateHostReleaseMetadata,
) bool {
	return got.Tag == want.Tag &&
		got.Commit == want.Commit &&
		got.PublishedAt.Equal(want.PublishedAt) &&
		got.ManifestAssetID == want.ManifestAssetID &&
		got.ManifestAssetName == want.ManifestAssetName &&
		got.ManifestSHA256 == want.ManifestSHA256 &&
		got.ManifestChecksumAssetID == want.ManifestChecksumAssetID &&
		got.ManifestChecksumSHA256 == want.ManifestChecksumSHA256 &&
		got.ArchiveAssetID == want.ArchiveAssetID &&
		got.ArchiveAssetName == want.ArchiveAssetName &&
		got.ArchiveSize == want.ArchiveSize &&
		got.ArchiveSHA256 == want.ArchiveSHA256 &&
		got.ArchiveChecksumAssetID == want.ArchiveChecksumAssetID &&
		got.ArchiveChecksumSHA256 == want.ArchiveChecksumSHA256 &&
		got.Arch == want.Arch &&
		got.AgentProtocolVersion == want.AgentProtocolVersion &&
		got.ExecutorProtocolVersion == want.ExecutorProtocolVersion &&
		got.MutationProtocolVersion == want.MutationProtocolVersion &&
		got.RecoveryProtocolVersion == want.RecoveryProtocolVersion &&
		got.MinimumPanelVersion == want.MinimumPanelVersion
}
