...
func TestTrustedPublicTagResolutionHandlesLightweightAnnotatedAndUnsafeChains(t *testing.T) {
	...
	for name, test := range map[string]struct {
		first updateHostBootstrapGitObject
		next  updateHostBootstrapGitObject
	}{
		"invalid commit": {
			first: updateHostBootstrapGitObject{Type: "commit", SHA: "not-a-commit"},
		},
		"cycle": {
			first: updateHostBootstrapGitObject{Type: "tag", SHA: tagOne},
			next:  updateHostBootstrapGitObject{Type: "tag", SHA: tagOne},
		},
	} {
		t.Run(name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
