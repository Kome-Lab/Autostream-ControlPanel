...
func (p SystemdPortReconfigurePlan) Validate() error {
	...
	if !remoteSessionPattern.MatchString(p.SessionID) ||
		!remotePlanHashPattern.MatchString(p.PortPlanSHA256) {
...
func (p SystemdPortReconfigurePlan) ComputePortPlanSHA256() (string, error) {
	if !identifierPattern.MatchString(p.JobID) ||
		...
		p.OwnershipEpoch < 1 ||
		p.LeaseGeneration == 0 ||
		!remoteSessionPattern.MatchString(p.SessionID) {
...
func (r SystemdPortReconfigureResult) Validate() error {
	...
	if mode == ModeSystemd {
		...
		if r.Result == systemdPortResultRollbackFailed {
			...
			!remotePlanHashPattern.MatchString(r.Docker.ComposeConfigSHA256) {
