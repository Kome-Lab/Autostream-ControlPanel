...
func validateHostPullClaim(job UpdateJob, serviceID string, binding HostAgentBinding, policy HostAgentPolicy) error {
	...
	if !identifierPattern.MatchString(job.ID) ||
		...
		!validRemoteSecret(job.LeaseToken) ||
...
func (a *HostPullAgent) invokePortExecutionMutation(
	...
	if operation == "port_reconfigure" {
		return a.PortExecutor.PortReconfigure(ctx, plan, fence, NewRemoteSecret(grant.Token))
	}
	return a.PortExecutor.PortReconfigureReconcile(ctx, plan, fence, NewRemoteSecret(grant.Token))
}
...
func (a *HostPullAgent) recoverExecutionPlan(policy HostAgentPolicy, job UpdateJob) (RemotePlan, error) {
	...
	if stored == nil || stored.Validate() != nil ||
		...
		return RemotePlan{}, errors.New("durable recovery plan does not match the recovered job")
	}
	rebound := *stored
...
func (a *HostPullAgent) prepareExecutionPlan(ctx context.Context, policy HostAgentPolicy, job UpdateJob) (RemotePlan, error) {
	target, ok := hostPullPolicyTarget(policy, job.TargetID)
	if !ok {
		return RemotePlan{}, errors.New("host pull policy target is unavailable")
	}
	if runtime.GOARCH != "amd64" && runtime.GOARCH != "arm64" {
		return RemotePlan{}, errors.New("host pull execution architecture is unsupported")
	}
	...
	if err := os.RemoveAll(verificationDir); err != nil {
		return RemotePlan{}, errors.New("clear incomplete host release verification")
	}
	applyPlan := ApplyPlan{
		JobID: job.ID, HostID: job.HostID, TargetID: job.TargetID,
	...
	default:
		return RemotePlan{}, errors.New("host pull target deployment mode is unsupported")
	}
...
func (a *HostPullAgent) invokeExecutionMutation(
	...
	if operation == "apply" {
		return a.Executor.Apply(ctx, plan, fence, NewRemoteSecret(grant.Token))
	}
	return a.Executor.Reconcile(ctx, plan, fence, NewRemoteSecret(grant.Token))
}
