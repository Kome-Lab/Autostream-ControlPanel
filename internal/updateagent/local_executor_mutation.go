...
func handleLocalExecutorMutation(
	...
	case "stage":
		...
		remote = remoteStageRequest(ctx, cfg, secured, *request.Plan, "", ledger, rt)
	case "apply", "reconcile":
		remote = remoteMutationRequest(ctx, cfg, secured, *request.Plan, request.Operation, request.MutationGrant, ledger, rt)
	default:
		return localExecutorFailureForVersion(LocalExecutorMutationProtocolVersion, "invalid_request")
	}
	return localExecutorResponseFromRemote(remote)
}
