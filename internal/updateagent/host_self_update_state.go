...
func ValidateHostRuntimeCompatibility(
	...
	if !updateHostBootstrapSemverAtLeast(current.AgentVersion, requirement.MinimumAgentVersion) {
		return fmt.Errorf("host agent %s is older than required %s", current.AgentVersion, requirement.MinimumAgentVersion)
	}
	if !updateHostBootstrapSemverAtLeast(current.ExecutorVersion, requirement.MinimumExecutorVersion) {
...
func (r HostSelfUpdateReleaseIdentity) validate() error {
	...
	if !versionPattern.MatchString(r.Tag) ||
		!updateHostBootstrapCommitPattern.MatchString(r.Commit) ||
		...
		!remotePlanHashPattern.MatchString(r.ManifestSHA256) ||
		!remotePlanHashPattern.MatchString(r.ManifestChecksumSHA256) ||
		r.ArchiveSize < 1 ||
		r.ArchiveSize > defaultMaxArtifactBytes ||
		!remotePlanHashPattern.MatchString(r.ArchiveSHA256) ||
		!remotePlanHashPattern.MatchString(r.ArchiveChecksumSHA256) ||
...
func (r HostSelfUpdateRequest) validate() error {
	if !identifierPattern.MatchString(strings.TrimSpace(r.Generation)) ||
		...
		!updateHostBootstrapCommitPattern.MatchString(strings.TrimSpace(r.Commit)) ||
