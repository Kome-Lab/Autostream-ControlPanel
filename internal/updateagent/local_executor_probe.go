...
func fetchLocalVersion(ctx context.Context, client *http.Client, endpoint string, target LocalExecutorTarget) (string, error) {
	...
	if err := fetchStrictLocalJSON(ctx, client, endpoint, &body, "version"); err != nil {
		return "", err
	}
	if !versionPattern.MatchString(strings.TrimSpace(body.Version)) ||
		body.ServiceID != target.ServiceID ||
		body.ServiceType != target.ServiceType ||
		body.ConfigRevision != target.ConfigRevision {
		return "", errors.New("version endpoint identity or config revision mismatch")
	}
