...
// initial bridge release. The server-owned epoch/policy still decides whether
// the returned agent remains observe-only or becomes mutation-capable.
func NewObserveOnlyHostAgent(bootstrap Config, options HostPullAgentOptions) (*HostPullAgent, error) {
	return NewHostPullAgent(bootstrap, options)
}
...
func validHostDockerPortObservation(observation HostDockerPortObservation) bool {
	return observation.CapabilityVersion == dockerPortCapabilityVersion &&
		...
		remotePlanHashPattern.MatchString(observation.ComposePolicySHA256) &&
