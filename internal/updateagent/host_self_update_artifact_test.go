...
func TestDownloadHostAgentReleaseBindsImmutableManifestAndBothBinaries(t *testing.T) {
	root := "autostream-host-agent_" + hostSelfUpdateArtifactTestVersion + "_linux_amd64"
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/repos/Kome-Lab/Autostream-ControlPanel/releases/tags/" + hostSelfUpdateArtifactTestVersion:
			names := []string{assetName, assetName + ".sha256", hostAgentManifestName, hostAgentManifestName + ".sha256"}
			assets := make([]updateHostBootstrapGitHubAsset, 0, len(names))
			for index, name := range names {
				sum := sha256.Sum256(payloads[name])
				assets = append(assets, updateHostBootstrapGitHubAsset{
					ID: int64(index + 1), Name: name,
					URL:    server.URL + "/repos/Kome-Lab/Autostream-ControlPanel/releases/assets/" + fmt.Sprint(index+1),
		...
		case "/repos/Kome-Lab/Autostream-ControlPanel/git/ref/tags/" + hostSelfUpdateArtifactTestVersion:
			fmt.Fprintf(w, `{"object":{"type":"commit","sha":%q}}`, hostSelfUpdateArtifactTestCommit)
		default:
			const prefix = "/repos/Kome-Lab/Autostream-ControlPanel/releases/assets/"
