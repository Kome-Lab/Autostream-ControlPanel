...
func TestUpdateHostBootstrapCertificateIdentityPolicy(t *testing.T) {
	...
	summary := trustedUpdateHostBootstrapCertificateSummary(identity)
	...
	tests := []struct {
	...
	}{
		{
			...
			mutate: func(summary *certificate.Summary) {
				summary.SubjectAlternativeName = strings.Replace(
					...
					updateHostBootstrapWorkflowPath,
		...
		},
		{
			name: "build signer",
			mutate: func(summary *certificate.Summary) {
				summary.BuildSignerURI = updateHostBootstrapRepositoryURI() +
...
func TestValidateUpdateHostBootstrapProvenanceResultPolicy(t *testing.T) {
	result, identity := trustedUpdateHostBootstrapVerificationResult(t, provenanceStatementFixture{})
	...
	tests := []struct {
		...
		},
		{
			name: "builder",
			mutate: func(fixture *provenanceStatementFixture) {
				fixture.builderID = updateHostBootstrapRepositoryURI() +
					"/.github/workflows/untrusted.yml@refs/tags/" +
					testBootstrapProvenanceVersion
			},
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			test.mutate(&fixture)
			result, identity := trustedUpdateHostBootstrapVerificationResult(t, fixture)
			if err := validateUpdateHostBootstrapProvenanceResult(
				result,
				identity,
				testBootstrapProvenanceVersion,
...
func TestUpdateHostBootstrapAttestationBundleURLPolicy(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			rawURL: "https://" + updateHostBootstrapBundleHost + "/sha256:abc/bundle.json",
		},
		{
			name:    "HTTP downgrade",
			rawURL:  "http://" + updateHostBootstrapBundleHost + "/sha256:abc/bundle.json",
			wantErr: true,
		},
		{
			name:    "foreign host",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := downloader.validateUpdateHostBootstrapAttestationBundleURL(test.rawURL)
...
func TestDownloadUpdateHostBootstrapAttestationBundleDecodesSnappyBundle(t *testing.T) {
	compressed := snappy.Encode(nil, []byte(testUpdateHostBootstrapSigstoreBundleJSON))
	headers := make(chan http.Header, 1)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		headers <- request.Header.Clone()
		w.Header().Set("Content-Type", updateHostBootstrapBundleMediaType)
	...
	decoded, err := downloader.downloadUpdateHostBootstrapAttestationBundle(
		context.Background(),
	...
	if string(decoded) != testUpdateHostBootstrapSigstoreBundleJSON {
		t.Fatal("decoded bundle does not match the compressed Sigstore fixture")
	}
	bundleHeaders := <-headers
	if accept := bundleHeaders.Get("Accept"); accept != updateHostBootstrapBundleMediaType {
	...
	if signedBundle, err := parseUpdateHostBootstrapAttestationBundle(decoded); err != nil {
...
func TestDownloadUpdateHostBootstrapAttestationBundleRejectsMediaTypeAndDecodedLimit(t *testing.T) {
	t.Run("media type", func(t *testing.T) {
		...
		_, err := downloader.downloadUpdateHostBootstrapAttestationBundle(
			context.Background(),
			server.URL+"/bundle.json.sn",
		)
	...
	t.Run("decoded size", func(t *testing.T) {
		compressed := snappy.Encode(
			nil,
			make([]byte, updateHostBootstrapDecodedMaxBytes+1),
		)
		_, err := decodeUpdateHostBootstrapAttestationBundle(compressed)
...
func TestListUpdateHostBootstrapAttestationsFollowsTrustedNextPage(t *testing.T) {
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		...
		if apiVersion := request.Header.Get("X-GitHub-Api-Version"); apiVersion != updateHostBootstrapAttestationAPI {
			t.Errorf("X-GitHub-Api-Version = %q", apiVersion)
		}
		if got := request.URL.Query()["per_page"]; len(got) != 1 ||
			got[0] != strconv.Itoa(updateHostBootstrapAttestationsPage) {
		...
		page := updateHostBootstrapAttestationList{}
		if cursor == "" {
			page.Attestations = make(
				[]updateHostBootstrapAttestation,
				updateHostBootstrapAttestationsPage,
			)
			next := *request.URL
			next.Scheme = "http"
			...
			page.Attestations = []updateHostBootstrapAttestation{
				{
					RepositoryID: updateHostBootstrapRepositoryID,
	...
	attestations, err := downloader.listUpdateHostBootstrapAttestations(
		context.Background(),
		testBootstrapProvenanceDigest,
	)
	if err != nil {
	...
	if len(attestations) != updateHostBootstrapAttestationsPage+1 {
		t.Fatalf("attestation count = %d", len(attestations))
	}
	if last := attestations[len(attestations)-1]; last.RepositoryID != updateHostBootstrapRepositoryID {
		t.Fatalf("last attestation = %#v", last)
	}
}

func TestListUpdateHostBootstrapAttestationsRejectsForeignNextPage(t *testing.T) {
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(updateHostBootstrapAttestationList{})
	}))
	defer server.Close()
	...
	_, err := downloader.listUpdateHostBootstrapAttestations(
		context.Background(),
		testBootstrapProvenanceDigest,
	)
...
func TestListUpdateHostBootstrapAttestationsDoesNotFollowRedirect(t *testing.T) {
	var foreignRequests atomic.Int32
	foreign := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		foreignRequests.Add(1)
...
func TestUpdateHostBootstrapAttestationNextLinkRejectsMalformedValues(t *testing.T) {
	if _, err := updateHostBootstrapNextLink([]string{"not-a-link"}); err == nil {
		t.Fatal("malformed Link header was accepted")
	}

	downloader := ReleaseDownloader{APIBase: "https://api.github.com"}
	initial, err := downloader.updateHostBootstrapAttestationEndpoint(
		testBootstrapProvenanceDigest,
	)
	if err != nil {
	...
	for _, rawURL := range tests {
		if _, err := validateUpdateHostBootstrapAttestationNextURL(initial, rawURL); err == nil {
			t.Fatalf("malformed next URL was accepted: %s", rawURL)
		}
	}
}

func TestUpdateHostBootstrapNextLinkIgnoresGitHubDeprecationLink(t *testing.T) {
	next, err := updateHostBootstrapNextLink([]string{
		`<https://docs.github.com/en/rest/about-the-rest-api/api-versions>; rel="deprecation"; type="text/html"`,
	})
	if err != nil {
...
func TestListUpdateHostBootstrapAttestationsFailsAtPageLimit(t *testing.T) {
	var requests atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		...
		w.Header().Set("Link", "<"+next.String()+`>; rel="next"`)
		_ = json.NewEncoder(w).Encode(updateHostBootstrapAttestationList{
			Attestations: []updateHostBootstrapAttestation{{RepositoryID: 1}},
		})
	}))
	defer server.Close()
	...
	if got := requests.Load(); got != updateHostBootstrapAttestationPages {
		t.Fatalf("requests = %d", got)
	}
}
...
func TestUpdateHostBootstrapProvenanceContextIsBounded(t *testing.T) {
	...
	child, childCancel, err := newUpdateHostBootstrapProvenanceContext(parent)
	if err != nil {
...
func TestUpdateHostBootstrapTUFFetcherEnforcesHTTPTimeout(t *testing.T) {
	...
	metadataFetcher := testUpdateHostBootstrapTUFFetcher(
		t,
		context.Background(),
		25*time.Millisecond,
		server,
...
func TestUpdateHostBootstrapTUFFetcherHonorsCallerCancellation(t *testing.T) {
	requestStarted := make(chan struct{})
	...
	metadataFetcher := testUpdateHostBootstrapTUFFetcher(
		t,
		ctx,
		updateHostBootstrapTUFHTTPTimeout,
		server,
	)
...
func TestUpdateHostBootstrapTUFFetcherHTTPPolicy(t *testing.T) {
	t.Run("User-Agent", func(t *testing.T) {
		...
		if userAgent := requestHeaders.Get("User-Agent"); userAgent != updateHostBootstrapTUFUserAgent {
	...
	t.Run("origin", func(t *testing.T) {
		metadataFetcher := newUpdateHostBootstrapTUFFetcher(
			context.Background(),
			time.Second,
		)
		tests := []string{
...
func testUpdateHostBootstrapTUFFetcher(
	t *testing.T,
	ctx context.Context,
	...
	metadataFetcher := newUpdateHostBootstrapTUFFetcher(ctx, timeout)
...
func trustedUpdateHostBootstrapCertificateSummary(identity verify.CertificateIdentity) certificate.Summary {
...
func trustedUpdateHostBootstrapVerificationResult(
	t *testing.T,
	fixture provenanceStatementFixture,
	...
	expectedWorkflowURI := updateHostBootstrapWorkflowURI(testBootstrapProvenanceVersion)
	if fixture.workflowPath == "" {
		fixture.workflowPath = updateHostBootstrapWorkflowPath
	}
	...
	if fixture.repositoryID == "" {
		fixture.repositoryID = strconv.FormatInt(updateHostBootstrapRepositoryID, 10)
	}
	...
	predicate, err := structpb.NewStruct(map[string]any{
		"buildDefinition": map[string]any{
			"buildType": updateHostBootstrapBuildType,
			...
			"internalParameters": map[string]any{
				"github": map[string]any{
					...
					"repository_owner_id": updateHostBootstrapRepositoryOwnerID,
	...
	statement := &intoto.Statement{
		...
		PredicateType: updateHostBootstrapSLSAPredicateType,
		Subject: []*intoto.ResourceDescriptor{
			{
				Name: updateHostBootstrapManifestName,
