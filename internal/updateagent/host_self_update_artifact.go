...
var hostAgentReleaseRepository = RepoSpec{
	Owner: "Kome-Lab", Repo: "Autostream-ControlPanel", Prefix: "autostream-host-agent",
}
...
func (d ReleaseDownloader) ResolveHostAgentReleaseMetadata(
	...
	manifestDigest, err := d.downloadUpdateHostBootstrapAsset(
	...
	manifestChecksumDigest, err := d.downloadUpdateHostBootstrapAsset(
	...
	archiveChecksumDigest, err := d.downloadUpdateHostBootstrapAsset(
...
// DownloadHostAgentRelease verifies the public immutable Control Panel
...
func (d ReleaseDownloader) DownloadHostAgentRelease(
	...
	artifactDigest, err := d.downloadUpdateHostBootstrapAsset(
...
func (d ReleaseDownloader) resolveHostAgentRelease(
...
) (resolvedUpdateHostBootstrapRelease, error) {
	...
	if !d.AllowHTTPForTest {
		...
		if err != nil || parsed.Scheme != "https" ||
			...
			return resolvedUpdateHostBootstrapRelease{}, errors.New("host self-update requires GitHub's production API origin")
		}
	}
	...
	if err := d.getJSON(ctx, endpoint, &release); err != nil {
		return resolvedUpdateHostBootstrapRelease{}, fmt.Errorf("resolve host agent release: %w", err)
	}
	...
	if !archiveOK || !checksumOK || !manifestOK || !manifestChecksumOK {
		return resolvedUpdateHostBootstrapRelease{}, errors.New("host agent release is incomplete")
	}
	convert := func(asset githubReleaseAsset) (updateHostBootstrapGitHubAsset, error) {
		converted := updateHostBootstrapGitHubAsset{
			ID: asset.ID, Name: asset.Name, URL: asset.URL,
			Digest: asset.Digest, State: asset.State,
		}
		if converted.State != "uploaded" ||
			!updateHostBootstrapAssetDigestPattern.MatchString(converted.Digest) {
			return updateHostBootstrapGitHubAsset{}, fmt.Errorf("host agent release asset %q has invalid immutable metadata", converted.Name)
		}
		if err := d.validateUpdateHostBootstrapAssetURL(converted, base); err != nil {
			return updateHostBootstrapGitHubAsset{}, err
		}
		return converted, nil
	}
	converted := make([]updateHostBootstrapGitHubAsset, 0, 4)
