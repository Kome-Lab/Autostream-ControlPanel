...
type RuntimeCredentialMutation struct {
	...
	RuntimeToken     RemoteSecret `json:"runtime_token,omitempty"`
}
...
func (m RuntimeCredentialMutation) validate(operation string) error {
	...
	if operation == "runtime_credential_stage" {
		if !validRemoteSecret(m.RuntimeToken.Reveal()) {
...
func runtimeCredentialStatusFromDirective(
	...
	return RuntimeCredentialStatus{
		...
		RotationRevision:            directive.Revision,
