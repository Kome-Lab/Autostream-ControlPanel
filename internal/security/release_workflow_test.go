...
func TestHostReleaseStagesVerifiesAndThenPublishes(t *testing.T) {
	...
	orderedContract := []string{
		...
		"root:root 0750",
		"non-root Host Agent securely loads and validates the canonical",
		"visible current and legacy pair",
		"Root-owned configure and Runtime Token rotation",
		"Control Panel ${RELEASE_VERSION} first",
		"Host Agent and Local Executor together",
		"Host Agent systemd unit remains unchanged",
		"do not rerun configure solely for this fix",
		"keep the exact access-only autostream-host-agent execute ACL through the matched upgrade",
		"candidate rollback can restart v1.9.9",
		"expected non-symlink parent",
		"never add a default, read, group, or mode grant",
		"never use setfacl -b",
