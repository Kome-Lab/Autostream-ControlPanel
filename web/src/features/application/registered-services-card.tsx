...
export function RegisteredServicesCard({
  ...
  return (
    <Card className="min-w-0">
      <CardHeader><CardTitle className="flex items-center gap-2"><ServerCog className="size-5" />{uiText("登録済みサービス")}</CardTitle><CardDescription>{uiText("報告バージョンと、希望・適用・Node報告のendpointを区別して表示します。")}</CardDescription></CardHeader>
      <CardContent>
        {!canViewNodeInfo ? <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">{uiText("登録済みNodeの情報を確認する権限がありません。管理者にNode情報の閲覧権限を依頼してください。")}</div>
          : nodesError ? <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950 dark:border-amber-900 dark:bg-amber-950/35 dark:text-amber-100"><span>{uiText("登録済みNodeの情報を取得できませんでした。通信状態とControl Panelのログを確認してください。")}</span><Button variant="outline" size="sm" onClick={onRefresh}>{uiText("再試行")}</Button></div>
            : nodesLoading ? <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">{uiText("読み込み中")}</div>
              : nodeRows.length === 0 ? <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">{uiText("登録済みNodeがありません。Node登録ページで作成したNodeがある場合は、ページを更新してください。")}</div>
                 : <>
                      <div className="grid gap-4 2xl:hidden">
                        <div className="space-y-2">
                          <SectionLabel title={uiText("Nodeサービス")} description={uiText("Worker、Encoder / Recorder、その他のサービス")} />
                          ...
                          ) : <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">{uiText("Nodeサービスは登録されていません。")}</div>}
                        </div>
                        {updaterRows.length > 0 ? (
                          <div className="space-y-2">
                            <SectionLabel title="Updater / Host Agent" description={uiText("ホスト単位の更新専用。通常のNode endpointとは別に管理します。")} />
                            <div className="grid gap-3">
                     ...
                     <div className="hidden overflow-x-auto rounded-md border 2xl:block">
                       <Table className="min-w-[980px]">
                         <TableHeader>
                           <TableRow>
                             <TableHead>{uiText("サービス")}</TableHead><TableHead>{uiText("種別 / バージョン")}</TableHead><TableHead>{uiText("状態")}</TableHead><TableHead className="min-w-80">{uiText("Endpoint / ポート変更")}</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                             ...
                             return (
                               ...
                               {(index === 0 || (nodeRows[index - 1]?.service_type === "update_agent") !== (node.service_type === "update_agent")) ? (
                                 <TableRow className="bg-muted/25 hover:bg-muted/25">
                                   <TableCell colSpan={4}>
                                     <SectionLabel title={node.service_type === "update_agent" ? "Updater / Host Agent" : uiText("Nodeサービス")} description={node.service_type === "update_agent" ? uiText("ホスト単位の更新専用") : uiText("Worker、Encoder / Recorder、その他のサービス")} />
                                   </TableCell>
                                 </TableRow>
                               ) : null}
                               <TableRow>
                                ...
                                <TableCell>
                                  ...
                                  <div className="mt-1">{node.reported_version || node.version || uiText("未報告")}</div>
                                ...
                                <TableCell className="space-y-2">
                                  ...
                                  <div><UpdateStatusBadge state={nodeUpdateState(node, serviceUpdateForNode(node, appVersion), uiText)} /></div>
                                </TableCell>
                                <TableCell>
                     ...
                     <p className="mt-3 text-xs text-muted-foreground">{uiText("「現在適用中」が実際の接続先です。「希望」は未適用の値を含み、Node報告と異なる場合があります。")}</p>
                   </>}
      </CardContent>
    </Card>
  );
  ...
  return (
    <article className="min-w-0 overflow-hidden rounded-md border bg-background/60 p-3">
      ...
      <div className="mt-3 grid min-w-0 gap-3 sm:grid-cols-2">
        <div className="min-w-0">
          <div className="text-xs text-muted-foreground">{uiText("種別 / バージョン")}</div>
          <div className="mt-1 break-words">{serviceTypeLabel(node.service_type)}</div>
          <div className="mt-1 break-words">{node.reported_version || node.version || uiText("未報告")}</div>
        ...
        <div className="min-w-0 space-y-2">
          <div className="text-xs text-muted-foreground">{uiText("状態")}</div>
          <StatusBadge status={node.health_status || node.status || "-"} />
          <div><UpdateStatusBadge state={nodeUpdateState(node, serviceUpdateForNode(node, appVersion), uiText)} /></div>
        </div>
      </div>
      <div className="mt-3 min-w-0 rounded-md border bg-muted/10 p-3">
        <div className="text-xs font-medium">{uiText("Endpoint / ポート変更")}</div>
