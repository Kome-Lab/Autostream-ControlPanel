...
function modeLabels(uiText: UICopy = japaneseCopy) { return Object.freeze({ system: uiText("システム"), light: uiText("ライト"), dark: uiText("ダーク") }); }
...
export function AppearancePanel() {
  ...
  const save = useMutation({
    ...
    onError: (error) => {
      ...
      setNotice(code === "revision_conflict" ? uiText("別の画面で更新されました。再読み込みしてから選び直してください。") : uiText("表示設定を保存できなかったため、直前の保存状態へ戻しました。"));
    },
  });
  ...
  const modeAccessibleName = (mode: UserColorMode) => locale === "en"
    ...
    : `${modeLabels(uiText)[mode]}モード`;

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-lg"><Palette className="size-5" />{uiText("外観")}</CardTitle>
        <CardDescription>{uiText("配色テーマと明るさを選択します。状態の意味を示す色は、どのテーマでも変わりません。")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        {preferenceQuery.isError ? <p role="alert" className="text-sm text-destructive">{uiText("保存済みの表示設定を読み込めませんでした。再読み込みしてください。")}</p> : null}
        {draft.fallback ? <p role="status" className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900 dark:bg-amber-950/30 dark:text-amber-200">{uiText("保存されたテーマを安全に表示できないため、AutoStream / システムで表示しています。DB値は自動変更していません。")}</p> : null}
        <fieldset disabled={preferenceQuery.isLoading || save.isPending}>
          <legend className="mb-2 text-sm font-medium">{uiText("テーマ")}</legend>
        ...
        <fieldset disabled={preferenceQuery.isLoading || save.isPending}>
          <legend className="mb-2 text-sm font-medium">{uiText("表示モード")}</legend>
        ...
        <div className="flex justify-end">
          <Button type="button" aria-label={uiText("表示設定を保存")} onClick={() => save.mutate()} disabled={!changed || preferenceQuery.isLoading || save.isPending}>
            <Save />{save.isPending ? uiText("保存中") : uiText("表示設定を保存")}
          </Button>
        </div>
      </CardContent>
    </Card>
