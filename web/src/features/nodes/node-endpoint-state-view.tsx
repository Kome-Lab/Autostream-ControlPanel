  ...
  if (state.kind === "pull_v2") {
    return (
      <div
        ...
        role="group"
        aria-label={uiText("Host Pull Agent transport情報")}
      >
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="secondary">transport_mode: {state.transportMode}</Badge>
          <span className="font-medium">{uiText("受信ポートなし（Outbound HTTPS）")}</span>
        </div>
        <div className="break-all text-muted-foreground">
          execution_host_id: {state.executionHostID || uiText("未報告")}
        </div>
        <div className="text-muted-foreground">
          ownership_epoch: {state.ownershipEpoch ?? uiText("未報告")}
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      className={compact
        ...
        : "grid min-w-0 gap-2 rounded-md border bg-muted/40 p-3 text-sm"}
      role="group"
      aria-label={uiText("Node endpoint migration状態")}
    >
      <div className="flex flex-wrap items-center gap-2">
        <Badge
          variant={state.status.tone}
          title={state.status.detail}
          aria-label={uiText("Endpoint状態: {0}。{1}", state.status.label, state.status.detail)}
        >
          {state.status.label}
        </Badge>
        <span className="text-muted-foreground">
          Revision {state.revision ?? uiText("未報告")}
        </span>
      </div>
  ...
  return (
    <div
      className={compact
        ...
        : "grid min-w-0 gap-2 rounded-md border bg-muted/40 p-3 text-sm"}
      role="group"
      aria-label={uiText("{0} のUpdater transport情報", node.service_name || node.service_id || node.id)}
    >
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant="secondary">Updater / Host Agent</Badge>
        <span className="font-medium">
          {isPull ? uiText("受信ポートなし（Outbound HTTPS）") : uiText("非対応transport")}
        </span>
      </div>
      <div className="text-muted-foreground">transport_mode: {transportMode}</div>
      {isPull ? (
        <>
          <div className="break-all text-muted-foreground">execution_host_id: {state.executionHostID || uiText("未報告")}</div>
          <div className="text-muted-foreground">ownership_epoch: {state.ownershipEpoch ?? uiText("未報告")}</div>
        </>
      ) : (
        <div className="break-all text-muted-foreground">
          {uiText("管理endpoint:")}{managementEndpoint || uiText("未報告")}
        </div>
      )}
      {!compact ? <div className="text-xs text-muted-foreground">{uiText("通常のNode endpointではなく、ホスト単位の更新経路です。")}</div> : null}
    </div>
  );
}
