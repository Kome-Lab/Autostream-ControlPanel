...
export function TableRecord<T>({ row, tableId }: { row: Row<T>; tableId: string }) {
  ...
  return <TableRow data-record-expanded={expanded}>
    {cells.map((cell) => {
      const presentation = cell.column.columnDef.meta as ColumnPresentation | undefined;
      const priority = presentation?.priority ?? 1;
      return <TableCell key={cell.id} className={cn(presentation?.className && "align-top", presentation?.className)} headers={`${tableId}-${cell.column.id}`} data-priority={priority > 1 ? "secondary" : "primary"}>
