...
test("fixed Relay Live API output asks for only nonsecret binding fields", () => {
  ...
  assert.match(formSource, /value: "live_api_relay_static", label: uiText\("YouTube Live API（固定Relay・既存互換）"\)/);
  ...
  assert.match(formSource, /<TextField label=\{uiText\("固定RelayバインディングID"\)\}[\s\S]*?required \/>/);
  assert.match(formSource, /<TextField label=\{uiText\("再利用するYouTube Live Stream ID"\)\}[\s\S]*?required \/>/);
...
test("YouTube output modes explain their relay compatibility", () => {
  assert.match(formSource, /value: "live_api", label: uiText\("YouTube Live API（本番・通常）"\), description: uiText\("通常はこちら。Control Panelが配信枠を作成し、EncoderからYouTubeへ直接送信します。固定Relayは不要です。"\)/);
  assert.match(formSource, /value: "live_api_dry_run", label: uiText\("YouTube Live API（検証）"\), description: uiText\("接続確認用です。実際に配信を開始する場合は本番・通常を選択してください。"\)/);
  assert.match(formSource, /value: "stream_key", label: uiText\("ストリームキー（従来方式）"\), description: uiText\("既存のストリームキー設定を使う場合だけ選択します。新規設定では本番・通常を推奨します。"\)/);
  assert.match(formSource, /const \[mode, setMode\] = useState\(\(\) => rowString\(row, \["mode", "config.mode"\]\) \|\| "live_api"\);/);
  assert.match(formSource, /\.\.\.\(staticRelayMode\r?\n\s*\? \[\{ value: "live_api_relay_static", label: uiText\("YouTube Live API（固定Relay・既存互換）"\)/);
});
...
test("normal Live API output can explicitly reuse a Studio custom stream key", () => {
  ...
  assert.match(formSource, /label=\{uiText\("Studioのカスタムキーを固定利用"\)\}/);
