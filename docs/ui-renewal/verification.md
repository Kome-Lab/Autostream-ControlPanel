...
## Convergence batch 021 — pointer settlement and usable panel focus region

- Continue the complete signed020 source, commit `69abbe37f0961f742b2dafc45c94df28a39934c5`, tree `bd53638de7e7a1b3a8004776b9c0aa26f2addeaf`. This candidate has no commit, signature, push, pin update or CI write. Changes are bounded by the Account input, current focus, lifecycle diagnosis and downstream validation causes.
- Account's actual loading status removal changed the same Appearance tab's vertical position between measurement and first native input. The visible-trigger helper retains the same target and original total deadline, samples actual frame geometry, revalidates unique/visible/enabled/clip/hit immediately before the single native click, and never resends an uncertain press. UI-POINTER-021 and UI-POINTER-SOCKET-021 preserve identity/negative/fatal and native socket boundaries. Local Chrome reached the first tab selection, preference save, 409 rollback and reload using the actual product. The past bootstrap cause remains UNDETERMINED; a successful immediate-mirror/delayed-DB reproduction does not erase it.
- The current observer compares a tall active tabpanel against usable height after actual sticky-header and ancestor clipping, not total viewport height alone. Reciprocal unique IDs, same selected Tabs owner, visible outline, edges and hit tests remain required; ordinary/inactive/hidden/inert/covered/wrong-reference controls are not exempt. Security's actual ResourcePage TabsContent receives the existing `scroll-mt-20` only when pageId is security. Account/Audit product code, common Tabs/TopBar/globals, tab stops and handlers stay unchanged. UI-PANEL-AVAILABLE-021, UI-SECURITY-PANEL-021 and the retained UI-PANEL-TAB-019 bind actual source/SSR/CSS/exercise/native socket and negatives. The old Security test's Account substitute is corrected while retaining the Account source assertion.
- Real focused Chrome Tab paths cover the twelve corresponding Audit/Security variants, and the unchanged Audit production export is exercised with the new observer. Development-server heading-origin checks are explicitly separate from the original full-document CI traversal; zero-size initial focus failures remain recorded. Existing harmless disclosure Enter/Space callbacks are tested with native input, zero business mutations and the unchanged activity-triggered session refresh. No activation pending is cleared merely because a different safe action passed.
- Current/history navigation reproduces a late current-generation Fetch pause without Network identity after Page.navigate replies. InvalidInterceptionId remains fatal, including required/current/unknown/POST cases. A deterministic real socket regression preserves the original cause and all waiters. Protected harness implementation stays unchanged; a bounded loader-correlation diagnostic proposal is private, not applied or executed. No stale ownership is inferred from matching frame/generation alone.
- All 28 signed020 executions are inventoried separately from candidate results. All 4,144 conditions/IDs/order/purposes, status-only40, old35 registrations and assertions, protected733 expectations, fixed history and exact package/lock with 454 manifests and HLS 1.7.0 stay fixed. Existing tests are not skipped or relaxed. Final source/binding/assembly/LOC, strict/noEmit/skipLibCheck:false, product typecheck, lint and required regression entries are bound to the frozen candidate; results and original intermediate failures are in the private RESULT.
- Previously skipped independent commands are executed where local prerequisites permit. Production build remains outside local policy. Local suites, focused Chrome results, original official CI, image/UI acceptance and Bundle completion remain separate. UA media/datetime internals, unregistered activation, native200%, Navigation API absent history and Preview/image acceptance remain pending. All9repo cleanup: NOT_STARTED.

- G4 downstream execution exposed two existing System Updates source checks that still expected untranslated JSX literals. Their original test names and safety assertions remain current; exact uiText calls replace the old literal syntax. UI-DOWNSTREAM-COPY-021 renders the actual runtime/Node components in both languages and rejects disconnected copy owners and a widened heartbeat maximum. Only the two corresponding B9-M015 destination hashes/LOC follow; no policy, normalizer, authentication, product authority or old expectation hash is changed.

## Continuation 022 — original entry verification and isolated ownership observation

- Retain all batch021 product, test and metadata changes. The formal candidate's protected harness and all protected expectation records remain unchanged. Only an isolated diagnostic copy enables Network events and records bounded exact-ID/session/frame/loader relations, event order, missing/late/evicted/ambiguous evidence and cleanup. Original fatal causes, legacy diagnostics, single settlement, cancellation rules and deadlines remain authoritative. Controlled negatives cover late events before and after failure, absent IDs, mismatches, redirection, capacity, observation/output failure and Network.enable setup failure.
- A finite baseline/observation comparison reproduces the original Account failure on the existing production export. Exact Network identity is observed in the diagnostic arm, but it grants no cancellation authority. A separate unapplied protected-control proposal describes lifecycle-issued document identities; its proposed after behavior is not executed or accepted evidence.
- The original registered35 entry completes locally through its unchanged Next server, including the complete Account fixture with immediate mirror assertion, delayed database result, native first click, save, conflict rollback, reload and English controls, plus the original downstream scenarios. The export-host trial's route-normalization failures and fatal remain separate evidence. No test assertion, request count, delay or product authentication behavior is changed to achieve the local completion.
- The same twelve focus IDs are attempted from the original document with native Tab and passive owner observation. Initial zero geometry is identified as a development portal shadow host containing the actually focused button. Original observer failures remain failures; no heading click, target exclusion, focus reset or timeout extension is added. Programmatic-server startup failure is also retained. These local environment results do not establish the twelve production conditions, native zoom or activation pending.
- Evidence fixes the browser entry, transitive private helpers, source and dependencies before execution. Unchanged batch021 source/command/dependency closures are reused explicitly, without adding historical test counts to new runs. New results, failures, preservation checks and the protected decision remain in one private continuation record. Existing official outcomes, fixed history, all4,144 objects/order/purposes, status-only40, UA/media/datetime pending and independent UI acceptance remain separate. All9repo cleanup: NOT_STARTED; no signing, commit, push or CI write.


## Continuation 024 — same-condition activation and isolated UA focus observation

- Continue the complete formal022/023 candidate. The original 4,144 objects, IDs, ordering and denominator, status-only40, old35 entry/fixtures, production source, package/lock and original protected expectations remain unchanged. The 480-ID worklist is an acceptance-blocker ledger, not a new official result. The current gate still requires every condition to PASS.
- The existing current exercise first executes its original keyboard path. Predeclared existing controls then provide missing Enter/Space activation on the same surface/state: account navigation menus, archive/metrics/audit/security selectors, pristine Node creation, local login theme preview, and restart confirmation Cancel with its exact opener. State change, unique connected target, trusted single key, original selection/mirror/focus restoration and the unchanged outer zero-business-mutation/unknown-API assertions are required. Existing disclosure/section paths remain intact. Missing controls are not N/A.
- The original path is the actual transitive `exerciseKeyboardPath` called by `exerciseAccessibility`; existing direct assertions remain attached to it. UI-ACTIVATION-024 and UI-ACTIVATION-MUTATION-024 cover the real wrapper and existing current mutation boundary, including replaced/hidden/disabled/ambiguous/covered targets, repeated or untrusted keys, unchanged state and failed restoration. Registration is not duplicated.
- The formal harness has no new control or observer API. A separate adoption trial adds a bounded typed read-only DOM/AX observer, linked through the same native Tab/Shift+Tab exercise. It checks the actual focused leaf against same document/frame/loader and UA host descendants, before/after identities, ambiguity, disabled/hidden state, paint and bounded pseudonymous trace. It never exposes raw protocol sends, names, values or AX trees, and cannot focus, scroll, mutate or play media. Its direct harness/socket tests include lookup/identity/bounds/output-cleanup failures. Existing per-host16 and total128 traversal limits, reverse order, both-direction escape, original host geometry/indicator and value invariants remain required.
- Only an observed same-condition UA descendant path may clear its corresponding identity reason. Host-only media, unavailable internal controls or a remaining independent reason stay NOT_PROVEN. Metadata observation is not playback, native200%, image/UI acceptance or official CI evidence. The final per-ID results and frozen inputs are recorded privately, separately from the old official status.
- The additional Account-on-static-export reproducer remains distinct from the original Next old35 and fixed historical images. An isolated already-owned static-prefetch drain experiment retained the original fatal/settlement tests but did not prevent the native error; it is rejected and excluded from the integrated adoption trial. No new cancellation exception, request-pattern omission, retry, deadline extension, fatal catch or protected expectation update is adopted. Previous failures remain intact.
- The unchanged production output and original Next35 evidence are reused only by matching source/closure/dependencies/command. New closure checks and native subset evidence are separate from those original runs. Fixed history build/export prerequisites, remaining media evidence and independent protected adoption remain explicit decisions. No commit/signature/push/pin/CI write is authorized. All9repo cleanup: NOT_STARTED.

## Continuation 025 — media state contracts and the readonly adapter

- Continue the full024 source. The sole adopted protected-harness supplement is the four reviewed readonly additions: typed import, same-session observer owner, fatal/closed-checked observation method and close-time clear. Removing those exact bytes restores the fixed approved Tab baseline; the original Tab transform then restores the immutable original. The protected733 records, original hashes, session/runner transforms, Fetch lifecycle, cancellation, deadlines and request ownership remain unchanged. The rejected Fetch trials are excluded.
- The observer returns bounded identity, disabled/host/descendant facts, the complete focusable set and media state. Initial and final document/frame/loader/host, focused leaf, subtree, complete set and state must agree. Available media still requires enabled internal controls with both-direction traversal, indicator, hit/clip, unchanged values and exit. Only independently registered unchanged Preview403/empty-source and public invalid-MP4 fixtures use the negative-state contract. Their real host must be visited, the complete set must contain only that disabled host, actual issue/settled403 or exact synthetic source must match, and trusted input must not play or issue another Preview request. Unknown/loading/wrong error, missing observations, changed ownership or an incomplete set fails.
- A disabled media host's native Space can scroll its real containing block. The negative-input driver observes same-owner scroll/scrollend and stable geometry within the existing deadline before subsequent Tab. On the public page it restores through the predeclared adjacent download anchor with one native Tab/Shift+Tab pair, counting both against the existing128-step budget. The exact original host must again have the same complete visible snapshot; no DOM focus, programmatic scroll, retry, suppressed default or visibility exemption is used. Mutation and unknown-request checks remain in the actual outer condition callback.
- Activation expectations follow the actual control: section/disclosure buttons keep Enter/Space each1, while the existing public download link requires Space0 and Enter1 default navigation to its exact synthetic endpoint. The narrow same-origin ready/public keyboard fixture provides a required settled GET204 with no body, preserving the document and avoiding a real file download. Other origins, paths, methods, queries and conditions are rejected. The original invalid MP4 and Preview403 are unchanged. Link event counts alone do not pass without actual request/response, target/document/focus and cleanup evidence.
- Existing test registrations and assertions remain. UI-MEDIA-025, UI-MEDIA-INPUT-025, UI-LINK-025 and UI-READONLY-025 connect actual observer/socket, emitted input expressions, state/identity faults, one-time native operations, restoration/cleanup failures and exact protected-delta negatives. Type corrections in the AX fixture keep the same final-frame negative. The private result distinguishes direct tests, complete local native conditions, reused unchanged evidence and original intermediate failures; overlapping runs are not added together.
- Process termination is not modified in this candidate. Its isolated passive trial and deterministic race comparisons preserve the original3s/2s waits, single taskkill/fallback and failure classification. A later successful cleanup is not a replacement for the original failure or proof of its cause. Fixed-history export/build prerequisites and old run-specific success/failure remain separate; current captures are not historical image comparisons.
- All4,144 condition objects/IDs/order, status-only40, old35 purposes, package/lock, authentication/permission/confirmation/normalizer and single-mutation/fresh-revalidation contracts are retained. Unavailable-media keyboard proof is not playback, native200%, image/UI acceptance or official CI success. No commit, signature, push or pin change; all9repo cleanup remains NOT_STARTED.

## Historical execution ownership

- The historical comparison entry verifies the original fixed helper checkout before creating a separate execution copy. An explicit, reversible transform changes only scenario ownership: each of the 32 independent entrances owns a fresh browser, while all 41 original captures retain their order, operations and assertions. Confirmation/cancel/focus/mutation, secret creation/disposal and error/recovery sequences remain in one owner. The fixed factory gives each browser separate request evidence; its existing server state is cloned across owners without aliasing, so old late callbacks cannot pollute new trace/counts. The original helper still performs phase preparation and trace/request/navigation/console resets. Each new document receives the original clock, locale and storage initialization, and every capture retains its original viewport. No arbitrary storage or secret is copied into a new profile.
- The execution copy never navigates a discarded product document to blank. It instead requires a newly owned, untouched blank target exactly once. The original protected harness, Fetch lifecycle, request resolver, deadlines, startup policy and cleanup remain byte-identical to the fixed source. No additional cancellation authority, request exclusion, settlement retry or error suppression is introduced. Required/current/unknown/POST and duplicate failures remain fatal; already registered stale requests retain their original contract.
- The current history entry emits the original and execution hashes, exact transform sites, unchanged-helper inventory and owner/capture correspondence. Reversing the allowed edits must reproduce the entire fixed original, including operation bodies, fixture calls, assertions, capture expressions and deterministic initialization. Both output inventories must exactly match the fixed 41-name order before unchanged API/DOM and zero-pixel comparison. Cleanup completes before success is written; simultaneous execution/cleanup failures preserve both causes.
- The existing history test entry retains its three original tests and registers direct transform, actual fixed-class/socket/fixture, real capture-caller and comparison negatives once. Controlled endpoint observations are not native PNG or browser evidence. Native before41/after41/comparison41 still requires exact exports of both fixed commits and the established official Linux/font conditions. Historical local full build is not made a delivery prerequisite. Current UI evidence is unaffected and is not rerun or counted as new history proof. No signing, commit, push, pin or CI write; all9repo cleanup remains NOT_STARTED.

## Historical owner asynchronous boundaries

- The history owner acquires an exclusive lease before launch and retains it through configuration, original operations, request-handler settlement and completed cleanup. Concurrent calls reject before another launch. Launch failure terminates the adapter; its original cause and the existing low-level startup policy remain unchanged.
- Shutdown permanently prevents new execution and waits for the entire pending lease. A late launch is closed exactly once without being configured or executed. An already executing operation retains its observations and handler settlement before cleanup. Repeated cleanup callers share the same completion or rejection, including simultaneous body/cleanup causes; the outer fixed capture caller preserves the original failure identity without duplicate aggregation.
- R028-OWNER-01 through05 require correct rejection and waiting for the five former boundary counterexamples. The sequential32/41 control, shutdown at setup/body/settlement boundaries, repeated failure, and real fixed-class/socket capture caller remain distinct local evidence. The actual caller test holds the first close response and verifies capture/required-response completion before the next launch. Original history assertions,14 transforms, fixed helper bytes,41 capture purposes and required official gates remain unchanged. Native41 is NOT_RUN without the exact export pair; controlled tests do not establish image or CI acceptance.
