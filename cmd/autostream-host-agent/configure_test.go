...
func TestRunHostAgentConfigureStagesWritesValidatesAndActivates(t *testing.T) {
	...
	dependencies.Stage = func(_ context.Context, panelURL, nodeID, token string, peer updateagent.HostAgentConfigurePeerIdentity, timeout time.Duration) (updateagent.UpdaterStagedConfiguration, error) {
		...
		if peer != (updateagent.HostAgentConfigurePeerIdentity{UID: 1001, GID: 1002}) {
			t.Fatalf("stage peer = %#v", peer)
		}
		stageToken = token
		return validHostAgentStagedConfiguration(), nil
	}
	dependencies.ValidateInstalled = func(path, policyPath string, staged updateagent.UpdaterStagedConfiguration) error {
		validatedPath = path
		if policyPath != updateagent.DefaultLocalExecutorPolicyPath ||
...
func TestRunHostAgentConfigureRejectsNonPullOrEndpointIdentityBeforeCommit(t *testing.T) {
	tests := []struct {
		name   string
		mutate func(*updateagent.UpdaterStagedConfiguration)
		want   string
	}{
		{
			name: "missing transport",
			mutate: func(staged *updateagent.UpdaterStagedConfiguration) {
		...
		},
		{
			name: "api endpoint",
			mutate: func(staged *updateagent.UpdaterStagedConfiguration) {
				staged.Config.API = updateagent.APIConfig{Host: "127.0.0.1", Port: 8090}
			},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			prepared := &fakePreparedHostAgentConfig{}
			dependencies := validHostAgentConfigureDependencies(prepared)
			dependencies.Stage = func(context.Context, string, string, string, updateagent.HostAgentConfigurePeerIdentity, time.Duration) (updateagent.UpdaterStagedConfiguration, error) {
...
func validHostAgentConfigureDependencies(prepared preparedHostAgentConfig) hostAgentConfigureDependencies {
	return hostAgentConfigureDependencies{
		...
		ValidateInstalled: func(string, string, updateagent.UpdaterStagedConfiguration) error { return nil },
		Activate: func(_ context.Context, _ string, staged updateagent.UpdaterStagedConfiguration, _ updateagent.UpdaterRuntimeReport, _ time.Duration) (updateagent.UpdaterActivationResult, error) {
			return updateagent.UpdaterActivationResult{State: "activated", ConfigurationID: staged.ConfigurationID}, nil
		},
...
func validHostAgentStagedConfiguration() updateagent.UpdaterStagedConfiguration {
	...
	return updateagent.UpdaterStagedConfiguration{
		...
		ConfigureProtocol:   updateagent.HostAgentConfigureProtocolVersion,
